<?php
/**
 * bressol.nl functions and definitions
 *
 * @link https://developer.wordpress.org/themes/basics/theme-functions/
 *
 * @package bressol.nl
 */

if (!defined('_S_VERSION')) {
    // Replace the version number of the theme on each release.
    define('_S_VERSION', '1.0.0');
}

/**
 * Sets up theme defaults and registers support for various WordPress features.
 */
if (!function_exists('bressol_nl_setup')) {
    function bressol_nl_setup() {
        // Make theme available for translation.
        load_theme_textdomain('bressol-nl', get_template_directory() . '/languages');

        // Add default posts and comments RSS feed links to head.
        add_theme_support('automatic-feed-links');

        // Let WordPress manage the document title.
        add_theme_support('title-tag');

        // Enable support for Post Thumbnails on posts and pages.
        add_theme_support('post-thumbnails');

        // This theme uses wp_nav_menu() in one location.
        register_nav_menus(
            array(
                'menu-1' => esc_html__('Primary', 'bressol-nl'),
            )
        );

        // Switch default core markup for search form, comment form, and comments to output valid HTML5.
        add_theme_support(
            'html5',
            array(
                'search-form',
                'comment-form',
                'comment-list',
                'gallery',
                'caption',
                'style',
                'script',
            )
        );

        // Set up the WordPress core custom background feature.
        add_theme_support(
            'custom-background',
            apply_filters(
                'bressol_nl_custom_background_args',
                array(
                    'default-color' => 'ffffff',
                    'default-image' => '',
                )
            )
        );

        // Add theme support for selective refresh for widgets.
        add_theme_support('customize-selective-refresh-widgets');

        // Add support for core custom logo.
        add_theme_support(
            'custom-logo',
            array(
                'height' => 250,
                'width' => 250,
                'flex-width' => true,
                'flex-height' => true,
            )
        );
    }
}
add_action('after_setup_theme', 'bressol_nl_setup');

/**
 * Set the content width in pixels, based on the theme's design and stylesheet.
 *
 * Priority 0 to make it available to lower priority callbacks.
 *
 * @global int $content_width
 */
function bressol_nl_content_width() {
    $GLOBALS['content_width'] = apply_filters('bressol_nl_content_width', 640);
}
add_action('after_setup_theme', 'bressol_nl_content_width', 0);

/**
 * Register widget area.
 *
 * @link https://developer.wordpress.org/themes/functionality/sidebars/#registering-a-sidebar
 */
function bressol_nl_widgets_init() {
    register_sidebar(
        array(
            'name' => esc_html__('Sidebar', 'bressol-nl'),
            'id' => 'sidebar-1',
            'description' => esc_html__('Add widgets here.', 'bressol-nl'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget' => '</section>',
            'before_title' => '<h2 class="widget-title">',
            'after_title' => '</h2>',
        )
    );
}
add_action('widgets_init', 'bressol_nl_widgets_init');

/**
 * Enqueue scripts and styles.
 */
function bressol_nl_scripts() {
    wp_enqueue_style('bressol-nl-style', get_stylesheet_uri(), array(), _S_VERSION);
    wp_style_add_data('bressol-nl-style', 'rtl', 'replace');

    wp_enqueue_script('bressol-nl-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);

    if (is_singular() && comments_open() && get_option('thread_comments')) {
        wp_enqueue_script('comment-reply');
    }
}
add_action('wp_enqueue_scripts', 'bressol_nl_scripts');

/**
 * Implement the Custom Header feature.
 */
require get_template_directory() . '/inc/custom-header.php';

/**
 * Custom template tags for this theme.
 */
require get_template_directory() . '/inc/template-tags.php';

/**
 * Functions which enhance the theme by hooking into WordPress.
 */
require get_template_directory() . '/inc/template-functions.php';

/**
 * Customizer additions.
 */
require get_template_directory() . '/inc/customizer.php';

/**
 * Load Jetpack compatibility file.
 */
if (defined('JETPACK__VERSION')) {
    require get_template_directory() . '/inc/jetpack.php';
}

// Incluir el WP_Bootstrap_Navwalker
require_once get_template_directory() . '/inc/class-wp-bootstrap-navwalker.php';

/**
 * Manejar la cookie de idioma
 */
add_action('init', 'set_language_cookie');
function set_language_cookie() {
    if (isset($_GET['lang'])) {
        setcookie('site_language', $_GET['lang'], time() + (86400 * 30), "/"); // 86400 = 1 day
        $_COOKIE['site_language'] = $_GET['lang']; // Para la solicitud actual
    }
}

/**
 * Mostrar el selector de idioma
 */
function display_language_selector() {
    $current_language = isset($_COOKIE['site_language']) ? $_COOKIE['site_language'] : 'en';
    ?>
    <select id="language-select">
        <option value="en" <?php selected($current_language, 'en'); ?>>English</option>
        <option value="nl" <?php selected($current_language, 'nl'); ?>>Dutch</option>
        <option value="es" <?php selected($current_language, 'es'); ?>>Spanish</option>
    </select>
    <script>
    document.getElementById('language-select').addEventListener('change', function() {
        var selectedLanguage = this.value;
        var currentUrl = window.location.href;
        var newUrl = currentUrl.replace(/\/(en|nl|es)\//, '/') + selectedLanguage + '/';
        window.location.href = newUrl;
    });
    </script>
    <?php
}
add_action('wp_footer', 'display_language_selector');

// Debugging de la cookie de idioma
add_action('wp_head', 'debug_language_cookie');
function debug_language_cookie() {
    error_log('Idioma actual en cookie: ' . (isset($_COOKIE['site_language']) ? $_COOKIE['site_language'] : 'No configurado'));
}
